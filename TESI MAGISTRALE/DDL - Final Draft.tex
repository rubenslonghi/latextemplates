...
\end{align}
where the pairing in the right-hand side is the one associated to forms living on $\partial M$ and where $\alpha\in\Omega^k(M)$ and $\beta\in\Omega^{k+1}(M)$ are arbitrary, though such that $\textrm{supp}(\alpha)\cap\textrm{supp}(\beta)$ is compact. 
...
\begin{remark}\label{Rmk: on nomenclature for Dirichlet and Neumann boundary conditions}
	...
	This will play a key r\^ole when we will verify that Assumption \ref{Thm: assumption theorem} is valid on static spacetime -- \textit{cf.} Proposition \ref{Prop: self-adjoint relation for parallel-, perp- and f,0- boundary conditions} in Appendix \ref{App: Existence of fundamental solutions on ultrastatic spacetimes}.
\end{remark}
...
We are grateful to Marco Benini, Sonia Mazzucchi, Jorma Louko, Valter Moretti, Ana Alonso Rodrigez, Alexander Schenkel and Alberto Valli for the useful discussions.
